...
int kBlockSize = 1 << 8;
int kGridSize =   kNumRow % kBlockSize == 0 ?
                            kNumRow / kBlockSize :
                            kNumRow / kBlockSize + 1;
...
void __parse_cli(int argc, char** argv){
    ...
    kGridSize = kNumRow % kBlockSize == 0 ?
                kNumRow / kBlockSize :
                kNumRow / kBlockSize + 1;
}
...
int main(int argc, char** argv){
    ...
    kIndexType *d_col_ids, *d_row_ptr;
    ...
    csr_spmv_kernel<kDataType><<<kGridSize, kBlockSize>>>(kNumRow, d_col_ids, d_row_ptr, d_data, d_x, d_y);
    PROFILE(
