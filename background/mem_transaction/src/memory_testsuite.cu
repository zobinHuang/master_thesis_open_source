...
template<typename dtype, int num_elements, int stride>
__global__ void issue_store(dtype *addr){
    ...
    for(int i=idx; i<num_elements; i+=blockDim.x){
        dtype *thread_addr = addr + i*stride;
        if constexpr (std::is_same<dtype, uint8_t>::value){
            asm volatile (
...
template<typename dtype, int num_elements, int num_iters, int stride>
...
template <typename dtype, int num_elements, bool aligned, int stride>
void start_store_test(std::fstream &f){
    ...
    dtype *addr, *launch_addr;
    l2flush l2;
    cudaEvent_t start, stop;
    float single_duration_ms;

    #define LAUNCH_STORE_KERNEL(addr) issue_store<dtype, num_elements, stride><<<BLOCK_COUNT, BLOCK_SIZE>>>(addr)

    // test eight times to take average
    for (int i = 0; i < 8; i++) {
        // initialize the destination vector
        if constexpr (aligned){
            GPU_ERROR(cudaMalloc(&addr, sizeof(dtype)*stride*num_elements));
            std::vector<dtype> array;
            for(int i=0; i<stride*num_elements; i++){
                if constexpr (std::is_same<dtype, uint8_t>::value){
                    array.push_back(1);
                } else if constexpr (std::is_same<dtype, float4>::value){
                    float4 *f = new float4();
            ...
            GPU_ERROR(cudaMemcpy(addr, array.data(), sizeof(dtype)*stride*num_elements, cudaMemcpyHostToDevice));
        } else {
            GPU_ERROR(cudaMalloc(&addr, sizeof(dtype)*stride*(num_elements+1)));
            std::vector<dtype> array;
            for(int i=0; i<stride*(num_elements+1); i++){
                if constexpr (std::is_same<dtype, uint8_t>::value){
                    array.push_back(1);
                } else if constexpr (std::is_same<dtype, float4>::value){
                    float4 *f = new float4();
            ...
            GPU_ERROR(cudaMemcpy(addr, array.data(), sizeof(dtype)*stride*(num_elements+1), cudaMemcpyHostToDevice));
        }
        ...
        LAUNCH_STORE_KERNEL(launch_addr);
...
template <typename dtype, int num_elements, int num_iters, bool aligned, int stride>
void start_load_test(std::fstream &f){
    MeasurementSeries duration_ms;
    ...
    #define LAUNCH_LOAD_KERNEL(addr) issue_load<dtype, num_elements, num_iters, stride><<<BLOCK_COUNT, BLOCK_SIZE>>>(addr)
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // measure duration
        GPU_ERROR(cudaEventRecord(start));
        LAUNCH_LOAD_KERNEL(launch_addr);
        GPU_ERROR(cudaEventRecord(stop));
        GPU_ERROR(cudaDeviceSynchronize());
        GPU_ERROR(cudaEventElapsedTime(&single_duration_ms, start, stop));
        duration_ms.add(single_duration_ms);
...
template <typename dtype, int num_iters, int test_type, bool aligned, int stride>
void measure() {
    int deviceId;
    cudaDeviceProp prop;
    std::string deviceName;
    std::fstream f;
    char log_file_name[1024]={0}, log_path_name[1024]={0};
    ...
    if(TEST_TYPE == TEST_TYPE_L1_LD || TEST_TYPE == TEST_TYPE_L2_LD || TEST_TYPE == TEST_TYPE_DRAM_LD) {            \
        start_load_test<dtype, NUM_ELE, num_iters, aligned, stride>(f);                                                     \
    } else if (TEST_TYPE == TEST_TYPE_L2_ST || TEST_TYPE == TEST_TYPE_DRAM_ST || TEST_TYPE == TEST_TYPE_L1_ST) {    \
        start_store_test<dtype, NUM_ELE, aligned, stride>(f);                                                               \
    }
    
    // create log folder for current device
    sprintf(log_path_name, "../log/%s", deviceName.c_str());
    if (!fs::is_directory(log_path_name) || !fs::exists(log_path_name)) {
        fs::create_directory(log_path_name);
    }
    
    #define WRITE_HEADER \
            /* basic setup */                                                                   \
        f   << "block_count " << "block_size " << "num_elements "                               \
            << "aligned " << "stride " << "duration "                                           \
            /* global metrics */                                                                \
    ...
    if(test_type == TEST_TYPE_L1_LD){
        if(deviceName == "Tesla V100-PCIE-32GB"){
            sprintf(
                log_file_name, "%s/understand_transaction_l1_ld_overall_%s_%d.txt",
                log_path_name,
    ...
    if(test_type == TEST_TYPE_L1_LD){
        if(deviceName == "Tesla V100-PCIE-32GB"){
            sprintf(
                log_file_name, "%s/understand_transaction_l1_ld_partial_%s_%d.txt",
                log_path_name,
                aligned ? "aligned" : "misaligned",
                stride
            );
    ...
    if(test_type == TEST_TYPE_L2_LD){
        if(deviceName == "Tesla V100-PCIE-32GB"){
            sprintf(
                log_file_name, "%s/understand_transaction_l2_ld_overall_%s_%d.txt",
                log_path_name,
                aligned ? "aligned" : "misaligned",
                stride
            );
    ...
    if(test_type == TEST_TYPE_DRAM_LD){
        if(deviceName == "Tesla V100-PCIE-32GB"){
            sprintf(
                log_file_name, "%s/understand_transaction_dram_ld_overall_%s_%d.txt",
                log_path_name,
                aligned ? "aligned" : "misaligned",
                stride
            );
    ...
    if(test_type == TEST_TYPE_L1_ST){
        if(deviceName == "Tesla V100-PCIE-32GB"){
            sprintf(
                log_file_name, "%s/understand_transaction_l1_st_%s_%d.txt",
                log_path_name,
                aligned ? "aligned" : "misaligned",
                stride
            );
    ...
    if(test_type == TEST_TYPE_L2_ST){
        if(deviceName == "Tesla V100-PCIE-32GB"){
            sprintf(
                log_file_name, "%s/understand_transaction_l2_st_%s_%d.txt",
                log_path_name,
                aligned ? "aligned" : "misaligned",
                stride
            );
    ...
    if(test_type == TEST_TYPE_DRAM_ST){
        if(deviceName == "Tesla V100-PCIE-32GB"){
            sprintf(
                log_file_name, "%s/understand_transaction_dram_st_%s_%d.txt",
                log_path_name,
                aligned ? "aligned" : "misaligned",
                stride
            );
...
int main(){
    ...
    // aligned test
    measure</* dtype */ float, /* num_iters */ (1<<3), /* test_type */ TEST_TYPE_L1_LD, /* aligned */ true, /* stride */ 1>();
    measure</* dtype */ float, /* num_iters */ (1<<3), /* test_type */ TEST_TYPE_L2_LD, /* aligned */ true, /* stride */ 1>();
    measure</* dtype */ float, /* num_iters */ (1<<3), /* test_type */ TEST_TYPE_DRAM_LD, /* aligned */ true, /* stride */ 1>();
    measure</* dtype */ float, /* num_iters */ 1, /* test_type */ TEST_TYPE_L1_ST, /* aligned */ true, /* stride */ 1>();
    measure</* dtype */ float, /* num_iters */ 1, /* test_type */ TEST_TYPE_L2_ST, /* aligned */ true, /* stride */ 1>();
    measure</* dtype */ float, /* num_iters */ 1, /* test_type */ TEST_TYPE_DRAM_ST, /* aligned */ true, /* stride */ 1>();

    // misaligned test
    measure</* dtype */ float, /* num_iters */ (1<<3), /* test_type */ TEST_TYPE_L1_LD, /* aligned */ false, /* stride */ 1>();
    measure</* dtype */ float, /* num_iters */ (1<<3), /* test_type */ TEST_TYPE_L2_LD, /* aligned */ false, /* stride */ 1>();
    measure</* dtype */ float, /* num_iters */ (1<<3), /* test_type */ TEST_TYPE_DRAM_LD, /* aligned */ false, /* stride */ 1>();
    measure</* dtype */ float, /* num_iters */ 1, /* test_type */ TEST_TYPE_L1_ST, /* aligned */ false, /* stride */ 1>();
    measure</* dtype */ float, /* num_iters */ 1, /* test_type */ TEST_TYPE_L2_ST, /* aligned */ false, /* stride */ 1>();
    measure</* dtype */ float, /* num_iters */ 1, /* test_type */ TEST_TYPE_DRAM_ST, /* aligned */ false, /* stride */ 1>();

    // non-continuous test
    // measure</* dtype */ float, /* num_iters */ (1<<3), /* test_type */ TEST_TYPE_L1_LD, /* aligned */ true, /* stride */ 2>();
    // measure</* dtype */ float, /* num_iters */ (1<<3), /* test_type */ TEST_TYPE_L2_LD, /* aligned */ true, /* stride */ 2>();
    // measure</* dtype */ float, /* num_iters */ (1<<3), /* test_type */ TEST_TYPE_DRAM_LD, /* aligned */ true, /* stride */ 2>();
    // measure</* dtype */ float, /* num_iters */ 1, /* test_type */ TEST_TYPE_L1_ST, /* aligned */ true, /* stride */ 2>();
    // measure</* dtype */ float, /* num_iters */ 1, /* test_type */ TEST_TYPE_L2_ST, /* aligned */ true, /* stride */ 2>();
    // measure</* dtype */ float, /* num_iters */ 1, /* test_type */ TEST_TYPE_DRAM_ST, /* aligned */ true, /* stride */ 2>();
