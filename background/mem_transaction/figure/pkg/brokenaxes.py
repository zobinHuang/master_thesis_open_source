...
class BrokenAxes:
        ...
        **kwargs
        ...
        for i, ax in enumerate(self.axs):
            if ylims is not None:
                ...
                ax.get_shared_y_axes().joined(ax, self.first_col[i // ncols])
            if xlims is not None:
                ax.set_xlim(xlims[i % ncols])
                ax.get_shared_x_axes().joined(ax, self.last_row[i % ncols])
    ...
    def text(self, x, y, s, *args, **kwargs):
        ...
        for ax in self.axs:
            ...
            def have_lim(lim_tuple:tuple):
                if(abs(lim_tuple[0]) < 0.1 and abs(lim_tuple[1]) < 0.1):
                    return False
                else:
                    return True

            if not have_lim(xlim) and have_lim(ylim):
                if ylim[0] < y < ylim[1]:
                    ax.text(x, y, s, *args, **kwargs)
                    return
            elif have_lim(xlim) and not have_lim(ylim):
                if xlim[0] < x < xlim[1]:
                    ax.text(x, y, s, *args, **kwargs)
                    return
            elif not have_lim(xlim) and not have_lim(ylim):
                ax.text(x, y, s, *args, **kwargs)
                return
            else:
