...
int main(int argc, char** argv){
    ...
    // generate row assignment scheme
    const std::vector<kIndexType> &row_assignment 
        = infer_row_assignment<kDataType, kIndexType, kBlockSize>(row_ptr);
    const int grid_size = row_assignment.size() - 1;

    // allocate memory space on device
    PROFILE(nvtxRangePush("allocate memory space on device");)
    kIndexType *d_col_ids, *d_row_ptr, *d_row_assignment;
    float *d_data, *d_x, *d_y;
    GPU_ERROR(cudaMalloc(&d_col_ids,            sizeof(kIndexType)*col_ids.size()));
    GPU_ERROR(cudaMalloc(&d_row_ptr,            sizeof(kIndexType)*row_ptr.size()));
    ...
    GPU_ERROR(cudaMalloc(&d_row_assignment,     sizeof(kIndexType)*row_assignment.size()));
    PROFILE(nvtxRangePop();)
    ...
    GPU_ERROR(cudaMemcpy(d_row_assignment, row_assignment.data(), sizeof(kIndexType)*row_assignment.size(), cudaMemcpyHostToDevice));
    PROFILE(nvtxRangePop();)
    ...
    csr_spmv_kernel<kDataType, kIndexType, kBlockSize><<<grid_size, kBlockSize>>>(kNumRow, d_row_assignment, d_col_ids, d_row_ptr, d_data, d_x, d_y);
    PROFILE(
    ...
    // verify result
    verify_spmv_result<kDataType, kIndexType>(kNumRow, col_ids, row_ptr, data, x, y);
